...
template <class T, class I, class Self> struct basic_promise {
  ...
  handle_type get_handle() {
    return handle_type::from_promise(*static_cast<Self *>(this));
  }
  T result_;
  I get_return_object() { return I{get_handle()}; }
  std::suspend_never initial_suspend() { return {}; }
  std::suspend_always final_suspend() noexcept { return {}; }
  template <std::convertible_to<T> U> void return_value(U &&value) {
    DEBUG(purple, "return_value called with ", orange, value, purple, " in ",
          get_handle().address());
    result_ = std::forward<U>(value);
  }
  void unhandled_exception() {}
  T &&result() && {
    DEBUG(purple, "returning ", orange, result_, purple, " in ",
          get_handle().address());
...
template <class P> struct resumable_handle {
  ...
  resumable_handle(resumable_handle &&other)
...
template <class T, class P> struct trivial_task : resumable_handle<P> {
  ...
  const T &&result() const && {
    return std::move(this->get_handle().promise().result);
  }
